...
def get_noise(input_depth, method, spatial_size, noise_type='u', var=1./1000, batch_size=1):
    """
    Versão adaptada para aceitar batch_size.
    Retorna um tensor de tamanho (batch_size x input_depth x H x W)
    """
    if isinstance(spatial_size, int):
        spatial_size = (spatial_size, spatial_size)
        
    if method == 'noise':
        # Trocamos o 1 fixo pelo parâmetro batch_size
        ...
        # Para o meshgrid, precisamos repetir a mesma grade para cada item do batch
        assert input_depth == 2
        X, Y = np.meshgrid(
            np.arange(0, spatial_size[1])/float(spatial_size[1]-1), 
            np.arange(0, spatial_size[0])/float(spatial_size[0]-1)
        )
        meshgrid = np.concatenate([X[None,:], Y[None,:]]) # Shape (2, H, W)
        
        # Converte para torch e adiciona a dimensão do batch repetindo os dados
        net_input = np_to_torch(meshgrid) # Shape (1, 2, H, W)
        if batch_size > 1:
            net_input = net_input.repeat(batch_size, 1, 1, 1) # Shape (N, 2, H, W)
    else:
        assert False
