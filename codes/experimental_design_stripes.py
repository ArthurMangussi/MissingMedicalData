...
def run_experimental_design(model_impt:str,
    ...
    for fold, (train_val_idx, test_idx) in enumerate(skf.split(images, labels)):
        ...
        x_train, x_train_md, _ = amputation.generate_stripes(x_train, frac_bad_cols=0.01, stripe_width=5)
        x_val, x_val_md, _ = amputation.generate_stripes(x_val, frac_bad_cols=0.01, stripe_width=5)
        x_test, x_test_md, missing_mask_test = amputation.generate_stripes(x_test, frac_bad_cols=0.01, stripe_width=5)
